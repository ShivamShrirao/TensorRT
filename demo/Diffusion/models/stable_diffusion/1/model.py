...
class TritonPythonModel:
    ...
    def initialize(self, args: Dict[str, str]) -> None:
        ...
        self.scheduler = EulerAncestralDiscreteScheduler.from_pretrained(self.scheduler_config_path)
    ...
    def prepare_mask_latents(
        self, mask, masked_image, pose_inputs, batch_size, height, width, dtype, device, generator, do_classifier_free_guidance
    ):
        ...
        inputs = [
            pb_utils.Tensor.from_dlpack(
                "images", torch.to_dlpack(masked_image)
            ),
        ]
        ...
        pose_inputs = torch.cat([pose_inputs] * 2) if do_classifier_free_guidance else pose_inputs
    ...
    def execute(self, requests) -> "List[List[pb_utils.Tensor]]":
        ...
        for request in requests:
            ...
            image = torch.from_dlpack(pb_utils.get_input_tensor_by_name(request, "IMAGE").to_dlpack())
            mask = torch.from_dlpack(pb_utils.get_input_tensor_by_name(request, "MASK").to_dlpack())
            pose_inputs = torch.from_dlpack(pb_utils.get_input_tensor_by_name(request, "POSE_INPUTS").to_dlpack())
            ...
            if scheduler.__class__.__name__ != scheduler:
                # TODO: Remove eval. It's a security risk.
                self.scheduler = eval(
                    f"{scheduler}.from_pretrained(self.scheduler_config_path)"
                )
            ...
            pose_inputs = pose_inputs.to(dtype=torch.float32) / 255.0
            ...
            inputs = [
                pb_utils.Tensor.from_dlpack(
                    "latent", torch.to_dlpack(latents)
                )
            ]
            inference_request = pb_utils.InferenceRequest(
            ...
            if inference_response.has_error():
                ...
                output = pb_utils.get_output_tensor_by_name(inference_response, "images")
                image: torch.Tensor = torch.from_dlpack(output.to_dlpack())
                image = (image / 2 + 0.5).clamp(0, 1) * 255
                image = image.type(dtype=torch.uint8)
